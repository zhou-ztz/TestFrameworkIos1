...
class TSGroupMembersViewController: TSViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        title = "chat_group_all_member".localized
        ...
        groupMembers.addObjects(from: Mapper<UserInfoModel>().mapArray(JSONObject: groupUsers)!)
        //初始化群主信息
        let memberId = self.originData["owner"]
        if Int(memberId as! String) == CurrentUserSessionInfo?.userIdentity {
            isGroupOwner = true
    ...
    func setMembersUI() {
        ...
        addButton.tag = 333
        addButton.setImage(UIImage.set_image(named: "btn_chatdetail_add"), for: .normal)
        ...
        deleteButton.setImage(UIImage.set_image(named: "btn_chatdetail_reduce"), for: .normal)
        ...
        for (index, item) in groupMembers.enumerated() {
            let userInfo = item as? UserInfoModel
...
//            let otherButtonX = space + otherWidth + CGFloat(index) * gSpace
            ...
            if userInfo?.avatarUrl != nil {
                faceButton.sd_setImage(
                    with: URL(string: (userInfo?.avatarUrl).orEmpty),
                    for: .normal,
                    placeholderImage: UIImage.set_image(named: "IMG_pic_default_secret"))
            } else {
                if userInfo?.sex == 1 {
                    faceButton.setImage(UIImage.set_image(named: "IMG_pic_default_man"), for: .normal)
                } else if userInfo?.sex == 2 {
                    faceButton.setImage(UIImage.set_image(named: "IMG_pic_default_woman"), for: .normal)
                } else {
                    faceButton.setImage(UIImage.set_image(named: "IMG_pic_default_secret"), for: .normal)
                }
            }
            ...
            if (userInfo?.verificationType).orEmpty.isEmpty {
                iconImage.isHidden = true
            } else {
                iconImage.isHidden = false
                if (userInfo?.verificationIcon).orEmpty.isEmpty {
                    switch (userInfo?.verificationType).orEmpty {
                    case "user":
                    ...
                    let urlString = (userInfo?.verificationIcon).orEmpty.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
            ...
            if userInfo?.userIdentity == Int("\(memberId ?? "")") {
                ...
                groupOwnerLabel.text = "chat_group_owner".localized
    ...
    @objc func deleteMember() {
        ...
        guard CurrentUserSessionInfo?.userIdentity == Int(groupOwnerID) else {
            return
        }
        let vc = TSChatFriendListViewController()
        ...
        vc.currenGroupId = self.currenGroupId
        vc.dataSource = self.groupMembers as! [UserInfoModel]
        ...
        for (_, item) in (changedMember?.enumerated())! {
            ...
            if currentAddOrDelete == "add" {
                ...
                for (originIndex, originItem) in groupMembers.enumerated().reversed() {
                    let originUserInfo = originItem as? UserInfoModel
    ...
    // MARK: - 进去个人中心
    @objc func interUsersHomePage(btn: UIButton) {
        let userInfo = groupMembers[btn.tag - 666] as! UserInfoModel
