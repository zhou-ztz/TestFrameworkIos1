...
class ReceivePendingTableVC: TSTableViewController, NoticePendingProtocol, TSCustomAcionSheetDelegate {
    ...
    lazy var indicator = TSIndicatorWindowTop(state: .loading, title: "bill_doing".localized)
    ...
    func setupUI() {
        ...
        buttonForTitle.setTitle("stick_type_group_commnet".localized, for: .normal)
        ...
        titleView.setTap { [weak self] (index) in // 标题菜单栏的点击事件
            ...
            guard let type = TSTopTargetType.Comment(rawValue: index) else { return }
            weakSelf.type = type
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        // 判断 是否已审核
        let sheetView = TSCustomActionsheetView(titles: ["choice_agree_set_to_top".localized, "choice_disagree_set_to_top".localized])
    ...
    func returnSelectTitle(view: TSCustomActionsheetView, title: String, index: Int) {
        switch title {
        case "choice_agree_set_to_top".localized:
            agreePending()
        case "choice_disagree_set_to_top".localized:
            denyPending()
        default:
    ...
    func agreePending() {
        ...
        TSPinnedNetworkManager.agreeCommentPinned(commentId: commentInfo.id, pinnedId: pendingData.id, commentTargetId: targetId, commentTargetType: self.type) { (msg, status) in
            ...
            if status {
                ...
                let resultIndicator = TSIndicatorWindowTop(state: .success, title: "text_already_approve".localized)
                resultIndicator.show(timeInterval: 2)
                self.tableView.reloadData()
            } else {
                let resultIndicator = TSIndicatorWindowTop(state: .faild, title: "network_problem".localized)
                resultIndicator.show(timeInterval: 2)
            }
        }
    }
    ...
    func denyPending() {
        ...
        TSPinnedNetworkManager.denyCommentPinned(commentId: commentInfo.id, pinnedId: pendingData.id, commentTargetId: targetId, commentTargetType: self.type) { (msg, status) in
            ...
            if status {
                self.pendingData.expiresDate = Date()
                let resultIndicator = TSIndicatorWindowTop(state: .success, title: "rejected".localized)
                resultIndicator.show(timeInterval: 2)
                self.tableView.reloadData()
            } else {
                let resultIndicator = TSIndicatorWindowTop(state: .faild, title: "network_problem".localized)
    ...
    func notice(pendingCell: NoticePendingCell, didClickRegion: NoticePendingCellClickRegion) {
        ...
        case .avatar, .title:
            FeedIMSDKManager.shared.delegate?.didClickHomePage(userId: model.userId, username: nil, nickname: nil, shouldShowTab: false, isFromReactionList: false, isTeam: false)
