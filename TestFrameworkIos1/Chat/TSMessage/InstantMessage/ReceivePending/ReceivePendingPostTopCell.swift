...
//        self.postControl = postControl
    ...
    func setupWithModel(_ model: ReceivePendingPostTopModel?) -> Void {
        ...
        self.timeLabel.text = TSDate().dateString(.normal, nDate: model.createDate)
        // 置顶积分/时间
...
//        }
        ...
        case .agree:
            title =  "review_approved".localized
            topInfoLabel.textColor = TSColor.normal.disabled
        case .reject:
            title =  "review_refuse".localized
            topInfoLabel.textColor = TSColor.normal.disabled
        }
        ...
        let strAtt = NSMutableAttributedString(string: "request_pin_post_in_circle".localized)
        strAtt.yy_setColor(TSColor.normal.minor, range: strAtt.yy_rangeOfAll())
        strAtt.yy_setFont(UIFont.systemFont(ofSize: 14), range: strAtt.yy_rangeOfAll())
        self.descLabel.attributedText = strAtt
        // post
...
//            // 帖子已删除的特殊处理
//            self.statusBtn.layer.borderColor = UIColor.clear.cgColor
//            self.statusBtn.setTitle( "post_has_been_deleted".localized, for: .normal)
//            self.statusBtn.snp.remakeConstraints { (mark) in
//                mark.trailing.equalToSuperview().offset(-lrMargin)
//                mark.centerY.equalTo(nameControl)
//                mark.height.equalTo(22)
