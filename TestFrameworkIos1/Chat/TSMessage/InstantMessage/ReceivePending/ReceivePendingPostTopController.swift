...
extension ReceivePendingPostTopController {
    ...
    fileprivate func showPostTopPopView(postId: Int) -> Void {
        ...
        alertVC.addAction(TSAlertAction(title: "choice_agree_set_to_top".localized, style: TSAlertSheetActionStyle.default, handler: { (action) in
            self.postTopAudit(postId: postId, audit: .agree)
        }))
        alertVC.addAction(TSAlertAction(title: "choice_disagree_set_to_top".localized, style: TSAlertSheetActionStyle.default, handler: { (action) in
...
//        let userVC = HomePageViewController(userId: userId)
