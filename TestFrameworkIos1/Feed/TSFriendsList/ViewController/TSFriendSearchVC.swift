...
class TSFriendSearchVC: TSViewController, UITableViewDelegate, UITableViewDataSource, TSFriendListCellDelegate, UITextFieldDelegate {
    ...
    func setUI() {
        ...
        occupiedView.center.y = occupiedView.center.y - 44
    ...
    func processRefresh(datas: [UserInfoModel]?, message: NetworkError?) {
        ...
        if message != nil {
            dataSource = []
            tableview.show(placeholderView: .network)
        }
...
//            AppDelegate.getHyPassword()
//            return
//        }
//        let idSt: String = String(userId)
//        let vc = ChatDetailViewController(conversationChatter: idSt)
