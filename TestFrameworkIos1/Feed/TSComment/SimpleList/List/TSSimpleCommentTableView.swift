...
protocol TSSimpleCommentTableViewDelegate: NSObjectProtocol {
    ...
    func didSelectCommentRow(CommentModel: FeedCommentListCellModel, sendType: SendCommentType)
...
class TSSimpleCommentTableView: UITableView, UITableViewDelegate, UITableViewDataSource, TSCustomAcionSheetDelegate, TSSimpleCommentTableViewCellDelegate {
    ...
    /// 评论数据
    var commentListDatas: [FeedCommentListCellModel] = Array() {
        didSet {
    ...
    ///   - width: 当前宽度
    /// - Returns: 高度
    private func setCommentHeight(comments: [FeedCommentListCellModel], width: CGFloat) -> CGFloat {
    ...
    ///   - style: 样式
    ///   - commentDatas: 数据
    init(width: CGFloat, commentDatas: [FeedCommentListCellModel]?, showCommentApplyTop: Bool = true) {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if userId == (CurrentUserSessionInfo?.userIdentity)! {
            ...
            switch commentData.sendStatus.rawValue {
            case 0:
            ...
            // Hide - YP-3830
           // let actionTitles = shouldHiddenTop ? ["choice_delete".localized] : ["choice_delete".localized]
            let actionTitles = shouldHiddenTop ? ["choice_delete".localized] : ["pin_comment_request".localized, "choice_delete".localized]
...
//            self.commentTableViewDelegate?.didSelectCommentRow(CommentModel:  commentListDatas[cellIndex!], sendType: .top)
    ...
    ///   - commnetObject: 评论的数据
    ///   - isRemove: 是否是删除
    public func addComment(commnetObject: FeedCommentListCellModel) {
    ...
    private func setFootView() -> UIView {
        ...
        let title = "display_see_all_comment".localized
    ...
    internal func repeatTap(cell: TSSimpleCommentTableViewCell, commnetModel: FeedCommentListCellModel) {
    ...
    func didTapToReply(cell: TSSimpleCommentTableViewCell, commnetModel: FeedCommentListCellModel) {
        self.commentTableViewDelegate?.didSelectCommentRow(CommentModel: commnetModel, sendType: .send)
    }
}
