...
protocol TSCommentListProtocol: class {
    ...
    /// 数据源列表
    var sourceList: [FeedCommentListCellModel] { get set }
    /// 高度数组
    ...
    /// 当前操作的评论
    var commentModel: FeedCommentListCellModel? { get set }
    /// 弹出键盘时相关的偏移
...
class TSCommentListController: TSViewController, TSListDataLoadProtocol, TSCommentListProtocol, TSDetailCommentTableViewCellDelegate {
    ...
    /// 数据源列表
    var sourceList: [FeedCommentListCellModel] = [FeedCommentListCellModel]()
    /// 高度数组
...
//                let faildList = DatabaseManager().commentManager.getAllFailedComments(type: self.type, sourceId: self.sourceId)
...
//            })
    ...
    func repeatTap(cell: TSDetailCommentTableViewCell, commnetModel: FeedCommentListCellModel) {
        ...
        if commnetModel.replyUserInfo != nil {
            self.commentModel = FeedCommentListCellModel()
        ...
        // 从数据库中删除
//        DatabaseManager().commentManager.deleteFailComment(commentId: commnetModel.id)
        // 重新发送
        self.sendComment(content, contentType: commnetModel.contentType.rawValue)
    }
    /// 点击了名字
...
//        self.navigationController?.pushViewController(userHomPage, animated: true)
    ...
    func didLongPressComment(in cell: TSDetailCommentTableViewCell, model: FeedCommentListCellModel) -> Void {
        if model.userId == CurrentUserSessionInfo?.userIdentity || model.userId == CurrentUserSessionInfo?.userIdentity {
            return
        }
...
extension TSCommentListController {
    ...
    @objc func initialUI() -> Void {
        self.navigationItem.title = "comment_list".localized
...
extension TSCommentListController {
    /// 发送评论
    @objc func sendComment(_ content: String, contentType: String) -> Void {
//        let replyId: Int? = self.commentModel?.userInfo?.userIdentity
//        TSCommentTaskQueue.submitComment(for: self.type, content: content, sourceId: self.sourceId, replyUserId: replyId, contentType: CommentContentType(rawValue: contentType) ?? .text) { (successModel, faildModel, msg, _) in
//            // 发送成功
...
//                successModel.user = CurrentUserSessionInfo?.convert()
//                // 当评论列表为空时，重定向请求位置
...
//            // 本地保存的发送失败的，从数据库中移除
//            DatabaseManager().commentManager.deleteFailComment(commentId: model.id)
...
//            self.showKeyBoard(placeHolderText: "reply_with_string".localized + "\((replyComment.userInfo?.name)!)", cell: cell)
//        } else {
//            self.commentModel = nil
//            self.showKeyBoard(placeHolderText: "placeholder_comment".localized, cell: nil)
//        }
    }
...
extension TSCommentListController {
    ...
    fileprivate func showCommentReportPopView(reportTarget: ReportTargetModel) -> Void {
        ...
        alertVC.addAction(TSAlertAction(title: "report".localized, style: TSAlertSheetActionStyle.default, handler: { (action) in
            let reportVC = ReportViewController(reportTarget: reportTarget)
...
//        TSKeyboardToolbar.share.keyboarddisappear()
//        if let commentUserId = comment.userInfo?.userIdentity, let userId = CurrentUserSessionInfo?.userIdentity, userId == commentUserId {
//            // 自己的评论，则弹出删除选项
//            self.index = indexPath.row
//            let alertVC = TSAlertController(title: nil, message: nil, style: .actionsheet)
//            alertVC.addAction(TSAlertAction(title: "choice_delete".localized, style: TSAlertSheetActionStyle.default, handler: { (_) in
//                self.showCommentDeleteConfirmAlert(commentIndex: self.index)
//            }))
//            // 评论申请置顶
//            if self.couldTopComment {
//                alertVC.addAction(TSAlertAction(title: "choice_request_comment_set_to_top".localized, style: TSAlertSheetActionStyle.default, handler: { (_) in
...
extension TSCommentListController: TSKeyboardToolbarDelegate {
    ...
    func keyboardToolbarSendTextMessage(message: String, bundleId: String?, inputBox: AnyObject?, contentType: CommentContentType) {
        ...
        self.sendComment(message, contentType: contentType.rawValue)
    }
