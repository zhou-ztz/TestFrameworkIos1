...
class TSDetailCommentCell: UITableViewCell {
    ...
    private func initialMainView(_ mainView: UIView) -> Void {
        ...
        // 4. topFlg
        let topImg = UIImage.set_image(named: "IMG_label_zhiding")!
        ...
        // 6. resendBtn
        let resendImage = UIImage.set_image(named: "IMG_msg_box_remind")!
    ...
    func setupWithModel(_ comment: TSCommentViewModel?) -> Void {
        ...
        self.layoutIfNeeded()
        self.headerIcon.avatarInfo = user.avatarInfo()
        self.nameControl.title = user.name
        if let date = comment.createDate {
            self.timeLabel.text = TSDate().dateString(.normal, nDate: date)
        } else {
            self.timeLabel.text = ""
        }
    ...
    func setupWithModel(_ comment: TSSimpleCommentModel?) -> Void {
        guard let comment = comment, let user = comment.userInfo else {
            return
        }
        self.topFlag.isHidden = !comment.isTop
