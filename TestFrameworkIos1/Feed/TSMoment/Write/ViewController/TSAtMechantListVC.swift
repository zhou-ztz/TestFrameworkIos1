...
class TSAtMechantListVC: TSViewController, UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource, TSMyFriendListCellDelegate {
    /// 数据源
    ...
    @objc func refresh() {
        TSUserNetworkingManager().mechantList(offset: nil, keyWordString: keyword, filterMerchants: "1", complete: { (userModels, networkError) in
            // 如果是第一次进入
    ...
    @objc func loadMore() {
        TSUserNetworkingManager().mechantList(offset: dataSource.count, keyWordString: keyword,  filterMerchants: "1", complete: { (userModels, networkError) in
