...
class AbstractRankingListTableViewCell: TSTableViewCell, BaseCellProtocol {
    ...
    /// 内容相对于上面的距离
    let contentLabelTop: CGFloat = 5.0
    /// 内容相对于右边的距离
    ...
    /// 最新关注按钮宽度
    let praiseButtonWidthNew: CGFloat = 49.0
    /// 最新关注按钮高度
    ...
    init(style: UITableViewCell.CellStyle, reuseIdentifier: String?, userInfo: UserInfoModel) {
        ...
        self.praiseButton?.contentEdgeInsets = UIEdgeInsets(top: 0, left: 8, bottom: 0, right: 8)
    ...
    // 数据加载
    fileprivate func setupWithModel(_ userModel: UserInfoModel) -> Void {
        self.headerImageButton?.avatarInfo = userModel.avatarInfo()
