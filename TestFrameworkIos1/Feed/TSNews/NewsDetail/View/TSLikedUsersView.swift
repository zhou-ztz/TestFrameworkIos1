...
class TSLikedUsersView: UIControl {
    ...
    /// 数据,资讯数据和点赞用户数据
    var data: (likeCount: Int, users: [UserInfoModel])? {
        didSet {
            updateData()
        }
    }
    ...
    func updateData() {
        ...
        for index in 0..<count {
            ...
            avatarButton.avatarInfo = users[index].avatarInfo()
        }
        usersCountLabel.text = String(format: "%d" + "event_detail_people_like".localized, data.likeCount)
