...
class FeedCommentLabel: TYAttributedLabel {
    ...
    func loadModel() {
        ...
        if model.showTopIcon {
            appendText("  ")
            append(UIImage.set_image(named: "IMG_label_zhiding"))
        }
    }
    ...
    func commentText() {
        // 1.设置文本对用的颜色字号
        let body = model.contentType == .sticker ? ":\n" : ":\n\(model.content)"
        
        var texts = [model.name, body]

        // MARK: REMARK NAME
        texts = [LocalRemarkName.getRemarkName(userId: "\(model.userId)", username: nil, originalName: model.name, label: nil), body]
    ...
    func comment(user replyName: String) {
        ...
        // 1.设置文本对用的颜色字号
        let texts = [model.name, " " + "reply_str".localized + " ", replyName, body]
    ...
    func contentImage() -> SDAnimatedImageView {
        ...
        imageView.sd_setImage(with: URL(string: model.content))
...
extension FeedCommentLabel: TYAttributedLabelDelegate {
    ...
    func attributedLabel(_ attributedLabel: TYAttributedLabel!, textStorageClicked textStorage: TYTextStorageProtocol!, at point: CGPoint) {
        guard model.contentType == .text else {
            feedCommentDelegate?.feedCommentListCellDidPress(self)
            return
        }
        // 1.获取点击文字内容
