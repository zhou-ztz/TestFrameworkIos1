...
class FeedCommentListView: UIView, UIGestureRecognizerDelegate {
    ...
    func set(datas: [FeedCommentListCellModel]) {
        ...
        // 2.查看全部按钮
        let buttonOrigin = CGPoint(x: 16, y: table.frame.maxY)
        if commentsCount > maxShowingCount && !datas.isEmpty {
            seeAllButton.setTitle("home_feed_view_all_comments".localized, for: .normal)
