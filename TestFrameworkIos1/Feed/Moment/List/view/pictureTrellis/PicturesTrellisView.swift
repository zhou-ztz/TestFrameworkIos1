...
class PicturesTrellisView: UIView {
    ...
    /// view, tapped picture index, transition id
    var onTapPictureView: ((PictureViewer, Int, String) -> Void)?
    ...
    /// 一张图宽度
    internal let width1: CGFloat = UIScreen.main.bounds.width
    /// 二分之一图的宽度
    internal let width2: CGFloat = (UIScreen.main.bounds.width - 1.0 * 3.5) / 2.0
    /// 三分之一图的宽度
    internal let width3: CGFloat = (UIScreen.main.bounds.width - 2.0 * 3.5) / 3.0
    /// 四分之一图的宽度
    internal let width4: CGFloat = (UIScreen.main.bounds.width - 3.0 * 3.5) / 4.0
    ...
    /// 设置视图
    func setUI() {
        for pictureIndex in 0...8 {
            let pictureView = PictureViewer()
            pictureViews.append(pictureView)
            pictureView.addTap { [weak self] (v) in
                guard let self = self else { return }
                let transitionId = UUID().uuidString
                pictureView.hero.id = transitionId
                self.onTapPictureView?(pictureView, pictureIndex, transitionId)
            }
        }
    }
    ...
    internal func loadModels() {
        ...
        for index in 0..<imageCount {
            ...
            if imageCount == 1 {
                pictureView.shouldAnimateGif = true
            } else {
                pictureView.shouldAnimateGif = false
            }
...
//            如果 宽大于高（横着的长方形） 播放器大小显示宽度width1*高度为原视频高度等比缩放后的高度 播放时内容等比缩放，完成填充内容播放
        ...
        if pictureOriginalSize.isLongPictureSize() {
            ...
            let buttonFrame = CGRect(x: 0, y: 0, width: width1, height: width1)
            //Temporarily use width1 as height because want to make landscape picture has same ratio with portrait picture https://yippiapp.atlassian.net/browse/SOCIAL-2288
            return buttonFrame
        }
    }
