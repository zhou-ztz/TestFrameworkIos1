//
//  FeedListCollectionLayout.swift
//  Yippi
//
...
@objc protocol FeedListCollectionLayoutDelegate: NSObjectProtocol {
    // 必选delegate实现
    /// collectionItem高度
    ...
    // 可选delegate实现
    /// 每个section 列数（默认2列）
    @objc optional func columnNumber(collectionView collection: UICollectionView, layout: FeedListCollectionLayout, section: Int) -> Int
    
    /// header高度（默认为0）
    @objc optional func referenceSizeForHeader(collectionView collection: UICollectionView, layout: FeedListCollectionLayout, section: Int) -> CGSize
    
    /// footer高度（默认为0）
    @objc optional func referenceSizeForFooter(collectionView collection: UICollectionView, layout: FeedListCollectionLayout, section: Int) -> CGSize
    
    /// 每个section 边距（默认为0）
    @objc optional func insetForSection(collectionView collection: UICollectionView, layout: FeedListCollectionLayout, section: Int) -> UIEdgeInsets
    
    /// 每个section item上下间距（默认为0）
    @objc optional func lineSpacing(collectionView collection: UICollectionView, layout: FeedListCollectionLayout, section: Int) -> CGFloat
    
    /// 每个section item左右间距（默认为0）
    @objc optional func interitemSpacing(collectionView collection: UICollectionView, layout: FeedListCollectionLayout, section: Int) -> CGFloat
    
    /// section头部header与上个section尾部footer间距（默认为0）
    @objc optional func spacingWithLastSection(collectionView collection: UICollectionView, layout: FeedListCollectionLayout, section: Int) -> CGFloat
}

class FeedListCollectionLayout: UICollectionViewFlowLayout {
    weak var delegate: FeedListCollectionLayoutDelegate?
...
extension FeedListCollectionLayout {
    ...
    override func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        if let delegate = self.delegate, let collectionView = self.collectionView {
            cellHeight = delegate.heightForRowAtIndexPath(collectionView: collectionView, layout: self, indexPath: indexPath, itemWidth: cellWeight)
        }
