...
class CreatTopicVC: TSViewController, TSCustomAcionSheetDelegate, UITextFieldDelegate, UITextViewDelegate, TZImagePickerControllerDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, TYAttributedLabelDelegate {
    ...
    private func initialUI() -> Void {
        ...
        // navigation bar
        if !isEditPush {
            self.navigationItem.title = "create_topic".localized
            let backItem = UIButton(type: .custom)
            backItem.addTarget(self, action: #selector(backItemClick), for: .touchUpInside)
            self.setupNavigationTitleItem(backItem, title: "cancel".localized)
            ...
            self.setupNavigationTitleItem(nextItem, title: "text_create".localized)
            ...
            self.navigationItem.title = "edit_feed_topic".localized
            self.navigationItem.hidesBackButton = true
            self.navigationItem.leftBarButtonItem = UIBarButtonItem(image: UIImage.set_image(named: "iconsArrowCaretleftBlack"), style: UIBarButtonItem.Style.plain, target: self, action: #selector(backItemClick))
            self.navigationItem.leftItemsSupplementBackButton = true
            let nextItem = UIButton(type: .custom)
            nextItem.addTarget(self, action: #selector(nextItemClick), for: .touchUpInside)
            self.setupNavigationTitleItem(nextItem, title: "choice_save".localized)
            self.navigationItem.rightBarButtonItem = UIBarButtonItem(customView: nextItem)
            nextItem.setTitleColor(UIColor.lightGray, for: .disabled)
            self.nextBtn = nextItem
        }
    ...
    func creatSubUI() {
        ...
        tyLabel.text = "      " + "create_topic_pic_hint".localized
...
//        self.view.addSubview(mengcengView)
        ...
        cameraIcon.image = UIImage.set_image(named: "ico_topic_camera")
        ...
        cameraLabel.text = "change_topic_pic_hint".localized
        ...
        titleField.placeholder = "text_enter_topic_title".localized
        ...
        placeHolderButton.text = "text_brief_topic_intro".localized
        ...
        tipsLabel.text = "text_create_success_title_cant_change".localized
        ...
        if isEditPush {
            if topicListModel.name != "" {
                if topicListModel.coverImage != nil {
                    faceImageView.sd_setImage(with: URL(string: TSUtil.praseTSNetFileUrl(netFile: topicListModel.coverImage).orEmpty))
                }
    ...
    @objc func backItemClick() {
        ...
        if !isEditPush && faceImageView.image == nil && titleField.text?.count == 0 && introtextView.text.count == 0 {
            ...
            if isEditPush {
                let alert = TSAlertController(title: "text_tips".localized, message: "edit_topic_cancle_tip".localized, style: .actionsheet, sheetCancelTitle: "text_continue".localized)
                alert.addAction(TSAlertAction(title: "edit_quit".localized, style: TSAlertSheetActionStyle.default, handler: { (_) in
                    self.backToFront()
                }))
                present(alert, animated: false, completion: nil)
            } else {
                let alert = TSAlertController(title: "text_tips".localized, message: "create_topic_cancle_tip".localized, style: .actionsheet, sheetCancelTitle: "text_continue".localized)
                alert.addAction(TSAlertAction(title: "create_topic_cancle".localized, style: TSAlertSheetActionStyle.default, handler: { (_) in
                    self.backToFront()
                }))
                present(alert, animated: false, completion: nil)
            }
    ...
    @objc func nextItemClick() {
        ...
        if isEditPush {
            // 3.发起网络请求
            let alert = TSIndicatorWindowTop(state: .loading, title: "save_pic_ing".localized)
            ...
            // 3.发起网络请求
            let alert = TSIndicatorWindowTop(state: .loading, title: "create_topic_doing".localized)
            if self.faceImageView.image != nil {
                guard let imageData = self.faceImageView.image!.jpegData(compressionQuality: 1.0) else {
                    self.nextBtn.isEnabled = true
                    return
...
//                }
    ...
    @objc func faceViewTap() {
        ...
        let personalIdentyAction = TSAlertAction(title: "choose_from_photo".localized, style: TSAlertSheetActionStyle.default, handler: { [weak self] (_) in
            self?.openLibrary()
        })
        let enterpriseIdentyAction = TSAlertAction(title: "choose_from_camera".localized, style: TSAlertSheetActionStyle.default, handler: { [weak self] (_) in
            self?.openCamera()
        })
    ...
    private func openLibrary() {
        ...
        /// #图片选择列表页面
        /// item右上角蓝色的选中图片
//            imagePickerVC.selectImage = UIImage.set_image(named: "msg_box_choose_now")
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        let type: String = infoDict.object(forKey: UIImagePickerController.InfoKey.mediaType) as! String
        if type == "public.image" {
            let photo: UIImage = infoDict.object(forKey: UIImagePickerController.InfoKey.originalImage) as! UIImage
    ...
    func imagePickerController(_ picker: TZImagePickerController!, didFinishPickingPhotos photos: [UIImage]!, sourceAssets assets: [Any]!, isSelectOriginalPhoto: Bool) {
        if photos.count > 0 {
            ...
            let resultAlert = TSIndicatorWindowTop(state: .faild, title: "error_picking_picture".localized)
            resultAlert.show(timeInterval: TSIndicatorWindowTop.defaultShowTimeInterval)
        }
    }
