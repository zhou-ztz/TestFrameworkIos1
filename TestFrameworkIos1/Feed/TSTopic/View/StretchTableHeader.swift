...
class StretchTableHeader: UIView {
    ...
    let topMaskView = UIImageView(image: UIImage.set_image(named: "pic_mask"))
    let bottomMaskView = UIImageView()

    init() {
        super.init(frame: .zero)
        if let image = UIImage.set_image(named: "pic_mask"), let cgImage = image.cgImage {
    ...
    func loadTable() {
        ...
        stretchModel.tableView.tableHeaderView = self
        stretchModel.tableView.contentInset = UIEdgeInsets(top: 0, left: stretchModel.tableView.contentInset.left, bottom: stretchModel.tableView.contentInset.bottom, right: stretchModel.tableView.contentInset.right)
...
extension StretchTableHeader {
    ...
    open func set(bgImageUrl: String?, placeholderImage: UIImage?, shouldCleanCache: Bool = false) {
        ...
        if shouldCleanCache {
            SDImageCache.shared.removeImage(forKey: bgUrl, withCompletion: nil)
        }
        bgImageView.sd_setImage(with: URL(string: bgUrl),
                                placeholderImage: stretchModel.placeholderImage)
    }
    ...
    @objc open func updateChildviews(tableOffset offset: CGFloat) {
        ...
        if let fixedView = stretchModel.fixedView {
            var yPos = frame.height - fixedView.frame.height
            stretchModel.fixedView?.frame = CGRect(origin: CGPoint(x: 0, y: yPos), size: fixedView.frame.size)
        }
    }
