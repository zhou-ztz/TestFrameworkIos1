...
class TopicListFixedHeaderView: UIView {
    ...
    func loadOwnerLabel(yRecord: inout CGFloat) {
        ...
        ownerLabel.text = String(format: "topic_creator".localized, model.ownerName)
    ...
    func loadTopicMenberView(yRecord: inout CGFloat) {
        ...
        let titleTextL = UILabel(frame: CGRect(x: 15, y: 19, width: ScreenWidth - 40, height: 15))
        titleTextL.textColor = UIColor(hex: 0x333333)
        titleTextL.font = UIFont.systemFont(ofSize: 13)
        titleTextL.text = "topics_people_involved".localized
    ...
    func setMenberUI(menber: [UserInfoModel], topY: CGFloat) {
        ...
        for (index, item) in menber.enumerated() {
            ...
            // MARK: REMARK NAME
            LocalRemarkName.getRemarkName(userId: nil, username: "\(item.name)", originalName: item.name, label: nameL)
