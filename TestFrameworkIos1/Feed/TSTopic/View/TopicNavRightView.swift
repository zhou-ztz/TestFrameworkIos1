...
class TopicNavRightView: UIView {
    ...
    func setUI() {
        // 1.搜索按钮
        searchButton.setImage(UIImage.set_image(named: "IMG_ico_search"), for: .normal)
        searchButton.sizeToFit()

        // 2.创建圈子按钮
//        buildButton.setImage(UIImage.set_image(named: "ico_create_topic"), for: .normal)
...
//        buildButton.frame = CGRect(origin: CGPoint(x: searchButton.frame.maxX + 12, y: 0), size: buildButton.size)
        frame = CGRect(origin: .zero, size: CGSize(width: searchButton.frame.maxX, height: searchButton.frame.height))
    }
}
