...
class TopicListHeaderView: StretchTableHeader {
    ...
    open func load(contentModel: TopicListControllerModel) {
        ...
        let tableYOffset = table.contentOffset.y > -(stretchHeaderModel.headerHeightMin) ? -(stretchHeaderModel.headerHeightMin) : table.contentOffset.y
        updateChildviews(tableOffset: tableYOffset)
        if contentModel.coverImage == nil {
