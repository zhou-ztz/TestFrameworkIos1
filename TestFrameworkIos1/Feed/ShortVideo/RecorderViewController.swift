...
class RecorderConsole: UIView {
    ...
    init(frame: CGRect, minDuration: CGFloat, maxDuration: CGFloat) {
        ...
        recorderBtn.setImage(UIImage.set_image(named: "ico_video_record"), for: .normal)
        ...
        finishRecorderBtn.setTitle("next_step".localized, for: .normal)
        ...
        undoRecorderBtn.setImage(UIImage.set_image(named: "ico_video_delete"), for: .normal)
        undoRecorderBtn.isHidden = true
        self.addSubview(undoRecorderBtn)

        whiteningBtn.setImage(UIImage.set_image(named: "ico_video_beauty_close"), for: .normal)
        whiteningBtn.setImage(UIImage.set_image(named: "ico_video_beauty_on"), for: .selected)
        whiteningBtn.frame = undoRecorderBtn.frame
        whiteningBtn.isHidden = false
        self.addSubview(whiteningBtn)

        reverseCameraBtn.setImage(UIImage.set_image(named: "ico_video_camera"), for: .normal)
...
class RecorderViewController: TSViewController {
    // MARK: - property
    /// 最小录制时长
...
//        previewView.addSubview(loadingView)
        ...
        backBtn.setImage(UIImage.set_image(named: "ico_video_close"), for: .normal)
...
//        audioConfig.format = Int32(kAudioFormatMPEG4AAC)
        ...
        focusView.outsideFocusTargetImage = UIImage.set_image(named: "ico_video_focusing")
...
//        recorder.pause {
//            if let session = self.recorder.session, session.segments.isEmpty {
...
//        prepareDuration(withSession: recorder.session)
    ...
    @objc func action(undoRecorderBtn btn: UIButton) {
        let actionsheetView = TSCustomActionsheetView(titles: ["confirm_delete_last_video".localized, "text_confirm".localized])
    ...
    @objc func popViewController(_ sender: UIButton) {
        ...
        let actionsheetView = TSCustomActionsheetView(titles: ["confirm_cancel_record_video".localized, "text_confirm".localized])
        actionsheetView.delegate = self
        actionsheetView.notClickIndexs = [0]
    ...
    @objc func action(recorderbtn recorderBtn: UIButton) {
        ...
        case .normal: // 按钮正常的情况下,被点击了一下,进入自动录制
            recorderBtn.setImage(UIImage.set_image(named: "ico_video_recording"), for: .normal)
    ...
    @objc func longPress(_ longPress: UILongPressGestureRecognizer) {
        ...
        if longPress.state == UIGestureRecognizer.State.began && type == .normal {
            btn.setImage(UIImage.set_image(named: "ico_video_recording"), for: .normal)
        ...
        if (longPress.state == .cancelled || longPress.state == .ended) && type == .manuRecording {
            btn.setImage(UIImage.set_image(named: "ico_video_record"), for: .normal)
