...
class HashTagFeedViewController: BaseFeedController {
...
//        feedShimmerView.bindToEdges()
    ...
    override func fetch() {
        HashtagRequest().getHashtagDetail(type: "all", hashtagId: self.hashtagId) { [weak self] (responseModels) in
...
//                    feedShimmerView.removeFromSuperview()
    ...
    override func loadMore() {
        ...
        HashtagRequest().getHashtagDetail(type: "all", hashtagId: self.hashtagId, limit: TSAppConfig.share.localInfo.limit, after: lastFeedId) { [weak self] (responseModels) in
            DispatchQueue.main.async {
