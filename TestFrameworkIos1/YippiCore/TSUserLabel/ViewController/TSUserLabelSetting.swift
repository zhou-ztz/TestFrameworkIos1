...
class TSUserLabelSetting: TSViewController, TSUserLabelCollectionViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = "event_edit_article_select_label".localized

        switch self.type {
        case .register:
            self.navigationItem.leftBarButtonItem = UIBarButtonItem(title: "", style: .plain, target: self, action: #selector(backItemClick))
            self.navigationItem.rightBarButtonItem = UIBarButtonItem(title: "skip".localized, style: .plain, target: self, action: #selector(rightItemClick))
    ...
    /// 显示温馨提示
    func showPrompt() {
        let alert = TSAlertController(title:  "tips".localized, message:  "tags_tips".localized, style: .alert)
        alert.addAction(TSAlertAction(title: "get_it".localized, style: TSAlertActionStyle.default, handler: nil))
        present(alert, animated: false, completion: nil)
    }

    // MARK: - 代理方法
    func dataSourceIsEmpty(isEmpty: Bool) {
        let title = isEmpty ? "skip".localized : "next_step".localized
    ...
    @objc private func backItemClick() -> Void {
        ...
        case .setting:
            ...
            let loadingAlert = TSIndicatorWindowTop(state: .loading, title:  "tag_setting".localized)
            loadingAlert.show()
            TSUserLabelNetworkManager.setUserTags(addTags: addTagIdList, deleteTags: deleteTagIdList, complete: { [weak self](msg, status) in
                ...
                if status {
                    ...
                    let alert: TSIndicatorWindowTop = TSIndicatorWindowTop(state: .faild, title:  "error_setting_tag".localized + (msg ?? ""))
