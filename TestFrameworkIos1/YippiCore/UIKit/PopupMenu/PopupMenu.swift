...
open class PopupMenu: UIView {
     ...
     */
    public var type : PopupMenuType = PopupMenuType.dark {
        
        didSet{
            switch type {
            case .dark:
                textColor = UIColor.white
                backColor = AppTheme.PopupMenu.backgroundColor
     ...
     */
   public static func showAtPoint(point : CGPoint,titles : Array<String>, icons: Array<String>?,menuWidth itemWidth : CGFloat,delegate : PopupMenuDelegate,otherSettings : (PopupMenu)->() ) -> PopupMenu {
        let popupMenu = PopupMenu.init()
        popupMenu.point = point
        popupMenu.titles = titles
    ...
    /**
     ...
     @param otherSetting   其他设置
     */
    public static func showAtPoint(point : CGPoint,titles : Array<String>, icons: Array<String>?,menuWidth itemWidth : CGFloat, didSelectRow : @escaping ((NSInteger,String,PopupMenu)->()), otherSettings : (PopupMenu)->() ) -> PopupMenu {
...
extension PopupMenu : UITableViewDelegate,UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        backgroundView.backgroundColor = AppTheme.PopupMenu.selectedBackgroundColor
        ...
        if images.count >= indexPath.row + 1 {
            if let cell = cell {
                cell.imageView?.image = UIImage(named: images[indexPath.row], in: Bundle.main, compatibleWith: nil)
            }
        }else{
