...
class TSIndicatorWindowTop: UIView {
    ...
    func setUI() {
        ...
        case .success: // 成功，显示蓝色图标
            let stateImage = UIImage.set_image(named: "ic_rl_srs_success")!
            imageSize = stateImage.size
            imageViewForState.image = stateImage
        case .faild: // 失败，显示红色图标
            let stateImage = UIImage.set_image(named: "IMG_msg_box_remind")!
            imageSize = stateImage.size
            imageViewForState.image = stateImage
        case .loading: // 加载中，显示小菊花
            var images: [UIImage] = []
            for index in 0...9 {
                let image = UIImage.set_image(named: "IMG_default_grey00\(index)")
