//
//  TSScrollSegmentView.swift
//  Yippi
//
//  Created by 深圳壹艺科技有限公司 on 2023/10/24.
//  Copyright © 2023 Toga Capital. All rights reserved.
//

import UIKit

open class TSScrollSegmentView: UIView {
    ...
    fileprivate lazy var scrollLine: UIView? = {[unowned self] in
        let line = UIView()
        line.layer.cornerRadius = CGFloat(self.segmentStyle.lineCornerRadius)
...
//MARK:- 逻辑处理
    public convenience init() {
          let frame = CGRect(x: 0, y: 0, width: 320, height: 44) // 默认的 frame
          let segmentStyle = SegmentStyle() // 提供一个默认的 SegmentStyle
          let titles = ["Title1", "Title2", "Title3"] // 默认的标题
          self.init(frame: frame, segmentStyle: segmentStyle, titles: titles)
    }
...
extension TSScrollSegmentView{
    ...
    fileprivate func setUpLabelsPosition() {
        ...
        for (index, item) in labelsArray.enumerated() {
            if let label = item as? CustomLabel {
                if index == 0 {
                    // 缩放, 设置初始的label的transform
                    if segmentStyle.scaleTitle {
                        label.currentTransformSx = segmentStyle.titleBigScale
                    }
                }
                
                // 设置初始状态文字的颜色
                label.textColor = index == 0 ? segmentStyle.selectedTitleColor : segmentStyle.normalTitleColor
            }
        }
    }
    /// 设置滚动条和遮罩
