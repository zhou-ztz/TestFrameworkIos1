...
class CameraConfig: NSObject {
    ...
    public func starSession(complete: @escaping(AVCaptureVideoPreviewLayer) -> Void) {
        cameraQueue.async { [weak self] in
            ...
            DispatchQueue.main.async() { [weak self] in
                ...
                weakSelf.preview?.videoGravity = AVLayerVideoGravity.resizeAspectFill
    ...
    public func swap(position newPosition: AVCaptureDevice.Position) {
    ...
    /// 设置 输入捕获
    private func configInput(position: AVCaptureDevice.Position) {
        // 1.获得摄像镜头
    ...
    private func getDevice(position currentPosition: AVCaptureDevice.Position) {
        guard #available(iOS 10, *) else {
        ...
        let deviceSession = AVCaptureDevice.DiscoverySession(deviceTypes: [.builtInWideAngleCamera], mediaType: AVMediaType.video, position: currentPosition)
        device = deviceSession.devices.filter { $0.position == currentPosition }.first
    }
}
