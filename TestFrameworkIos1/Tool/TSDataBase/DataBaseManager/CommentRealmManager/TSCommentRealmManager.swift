...
extension TSCommentRealmManager {
    ...
    func get(type: TSCommentType, sourceId: Int) -> [TSCommentModel] {
        ...
        for commentModel in commentList {
            commentModel.user = UserInfoModel.retrieveUser(userId: commentModel.userId)
            if let replyUserId = commentModel.replyUserId {
                commentModel.replyUser = UserInfoModel.retrieveUser(userId: replyUserId)
            }
        }
        return commentList
    }
...
extension TSCommentRealmManager {
    ...
    /// 删除指定的失败的评论
    func deleteFailComment(commentId: Int) -> Void {
    ...
    fileprivate func delete(_ comment: TSFailedCommentModel) -> Void {
        self.deleteFailComment(commentId: comment.id)
    }
