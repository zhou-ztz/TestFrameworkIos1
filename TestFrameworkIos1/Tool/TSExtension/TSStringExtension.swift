...
extension String {
    ...
    func ts_standardMarkdownToNormal() -> String {
        ...
        string.replaceAll(matching: imgRegex, with: "recent_msg_desc_picture".localized)
        return string
    }
    ...
    func modMarkdownString(for content: String) -> String {
        ...
        do {
            ...
            let modString = regex.stringByReplacingMatches(in: myString, options: [], range: range, withTemplate: String(format:"![$1](%@/$2)", url))
            return modString
        } catch {
            return content
        }
    ...
    func ts_customMarkdownToStandard() -> String {
        return modMarkdownString(for: self)
    ...
    func ts_convertStandardMarkdownToAttributeString(contentW: CGFloat, font: UIFont?, color: UIColor?) -> NSAttributedString? {
        ...
        while imgRegex.matches(string) {
            ...
            // 获取缓存中的图片 并 作为附件添加
            let image: UIImage? = SDImageCache.shared.imageFromCache(forKey: strImageUrl)
