...
extension FileManager {
    ...
    func getDirPath(for name: String, ext: String?) -> URL? {
        guard let url = urls(for: .documentDirectory, in: .userDomainMask).first else { return nil }
        if let ext = ext {
            return url.appendingPathComponent(name).appendingPathExtension(ext)
        }
        return url.appendingPathComponent(name)
    }
    
    func fileExist(for name: String, ext: String?) -> Bool {
        guard let url = getDirPath(for: name, ext: ext) else { return false }
