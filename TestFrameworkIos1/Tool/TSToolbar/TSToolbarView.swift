...
class TSToolbarView: UIView, TSToolBarButtonItemDelegate {
    ...
    /// 工具栏按钮的主题色
    private var itemTintColor: UIColor = .clear
    ...
    func setUI() {
        ...
        for (index, model) in itemModels!.enumerated() {
            ...
            if TSUserInterfacePrinciples.share.hasNotch() == true {
                height = 48.0
            }
            var width: CGFloat = 0
            var x: CGFloat = 0
            ...
            case .left: // item 图片在左文字在右时
                ...
                if model.titleShouldHide == true {
                    width = CGFloat(24.0 + 30.0) // 这里 width 的值是根据设计图配置的
                } else {
                    width = CGFloat(24.0 + 40.0) // 这里 width 的值是根据设计图配置的
                }
                
                if model.index == 3 {
                    x = self.bounds.width - width
                } else {
                    x = (self.subviews.last?.frame.maxX).orZero
                }
                // 如果 leftPadding 有值，并且当前 index 为 0
                if leftPadding != 0 && model.index == 0 {
                    x += leftPadding
                }
            case .top:
    ...
    // MARK: - Private
    /// 通过 index 获取 item
    func getItemAt(_ index: Int) -> TSToolBarButtonItem {
    ...
    public func item(isHidden: Bool, at index: Int) {
        if index == 1 {
            ...
            let item0 = getItemAt(0)
            let item2 = getItemAt(2)
            if isHidden {
                if !item0.isHidden {
                    item0.left = 0
                }
                if !item2.isHidden {
                    item2.left = getItemAt(0).frame.maxX
                }
                
            } else {
                item2.left = getItemAt(1).frame.maxX
            }
