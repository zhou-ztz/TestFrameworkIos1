...
private struct TSToolBarButtonItemUX {
    /// 图片长宽
    static let imageLenth: CGFloat = 24.0
    /// 图片和文字之间的间距
...
class TSToolBarButtonItem: UIView {
    ...
    func setUI() {
        ...
        if model?.titleShouldHide == false {
    ...
    func updateChildView() {
        ...
        // 设置 image view 共有配置
        imageView.image = UIImage.set_image(named: model!.image!)
        ...
        if model?.index == 3 && type == .left {
            imageView.frame = CGRect(x: self.bounds.width - imageWidth, y: (self.bounds.height - imageWidth) / 2, width: imageWidth, height: imageWidth)
            return
        }
