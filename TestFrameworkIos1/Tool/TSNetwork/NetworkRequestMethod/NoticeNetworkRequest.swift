...
struct UnreadPinnedsModel: Mappable {
    ...
    mutating func mapping(map: Map) {
        ...
        newsDate <- (map["news.time"], DateTransformer)
        feedsCount <- map["feeds.count"]
        feedsDate <- (map["feeds.time"], DateTransformer)
        postCount <- map["group-posts.count"]
        postDate <- (map["group-posts.time"], DateTransformer)
        postCommentCount <- map["group-comments.count"]
        postCommentDate <- (map["group-comments.time"], DateTransformer)
    }
}
...
struct UnreadDetailInfoUserModel: Mappable {
    ...
    var time: Date = Date()
    /// 用户
    var user: UserInfoModel = UserInfoModel()

    init?(map: Map) {
    }
    mutating func mapping(map: Map) {
        time <- (map["time"], DateTransformer)
        user <- map["user"]
    }
}
...
struct UnreadAtMeInfoModel: Mappable {
    ...
    mutating func mapping(map: Map) {
        time <- (map["latest_at"], DateTransformer)
        userNames <- map["users"]
    }
}
struct UnreadSystemInfoModel: Mappable {
    ...
    mutating func mapping(map: Map) {
        time <- (map["created_at"], DateTransformer)
...
//    var createdDate: Date!
    ...
    mutating func mapping(map: Map) {
//        id <- map["id"]
//        readDate <- (map["read_at"], DateTransformer)
//        detail <- map["data"]
