...
struct UserNetworkRequest {
    ...
    let blackList = Request<UserInfoModel>(method: .get, path: "user/blacks", replacers: [])
    // MARK: 新增数据
    ...
    /// 用户yipps
    let wallet = Request<UserWalletIntegrationModel>(method: .get, path: "user/currency", replacers: [])
    
    let tagged = Request<FeedListResultsModel>(method: .get, path: "feeds/tagged", replacers: [])
    
    let gallery = Request<FeedListResultsModel>(method: .get, path: "users/{user_id}/gallery", replacers: ["{user_id}"])
    
    let miniVideo = Request<FeedListResultsModel>(method: .get, path: "users/{user_id}/mini-video", replacers: ["{user_id}"])
}
...
struct UserCounts: Mappable {
    ...
    struct At: Mappable {
        ...
        mutating func mapping(map: Map) {
            badge <- map["badge"]
            last_created_at <- (map["last_created_at"], DateTransformer)
    ...
    struct System: Mappable {
        ...
        struct First: Mappable {
            ...
            mutating func mapping(map: Map) {
                id <- map["id"]
                created_at <- (map["created_at"], DateTransformer)
                data <- map["data"]
            }
        }
    }
