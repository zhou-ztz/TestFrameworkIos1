...
struct TSRewardNetworkRequest {
    ...
    struct Moment {
        ...
        /// 打赏
        static let reward = Request<Empty>(version: .wallet, method: .post, path: "feeds/{feed}/tips", replacers: ["{feed}"])
...
//        GET /api/v2/question-answers/:answer/rewarders
    ...
    struct User {
        /// 打赏
        static let reward = Request<Empty>(version: .wallet, method: .post, path: "users/:user/tips", replacers: [":user"])
    }

    struct Sticker {
        static let reward = Request<Empty>(version: .wallet, method: .post, path: "stickers/:sticker/tips", replacers: [":sticker"])
    }
...
//        POST plus-group/group-posts/:post/rewards
    ...
    struct Live {
        static let reward = Request<Empty>(version: .wallet, method: .post, path: "feeds/{feed}/tips/live", replacers: ["{feed}"])
    }
}
