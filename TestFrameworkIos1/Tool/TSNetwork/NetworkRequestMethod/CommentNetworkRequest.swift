...
struct CommentNetworkRequest {
    ...
    struct Moment {
        ...
        static let pinComment = Request<Empty>(method: .post, path: "feeds/:feed/comments/:comment/pinned", replacers: [":feed", ":comment"])
        
        static let unpinComment = Request<Empty>(method: .delete, path: "feeds/:feed/comments/:comment/unpinned", replacers: [":feed", ":comment"])
