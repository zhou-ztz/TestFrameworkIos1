...
class TSCommentNetWorkManager: NSObject {
    ...
    ///   - feedId: feedId
    ///   - complete: 返回评论列表的数组
    func getCommetList(feedId: Int, maxId after: Int?, complete: @escaping((_ isSuccess: Bool, _ commnetModels: [TSMomentCommentModel]?, _ error: String?) -> Void)) {
        // 配置 path
        ...
        try! RequestNetworkData.share.textRequest(method: .get, path: path, parameter: nil, complete: { (data: NetworkResponse?, status: Bool) in
            ...
            guard let datas = data as? [String: Any] else {
                complete(status, nil, "error_data_server_return".localized)
                return
            }
            // 3. 正常解析数据
            ...
            // 3.3.解析数据
            var commentObject: [TSMomentCommentModel] = []
            for index in 0..<commentDatas.count {
                let comment = commentDatas[index]
                var commentModel = TSMomentCommentModel(comment)
                if index >= normals.count {
                    commentModel.painned = 1
                }
                var isHave = false
                let deleteCommentsTask = DatabaseManager().comment.getDeleteTask()
    ...
    ///   - feedId: 动态的唯一Id
    ///   - complete: 返回的参数
    func send(commentContent: String, replyToUserId: Int?, feedId: Int, type: ReceiveInfoSourceType, contentType: CommentContentType, complete: @escaping((_ message: String?, _ id: Int?, _ error: NSError?) -> Void)) {
        let sourceId = feedId
        let commentType: TSCommentType = TSCommentType(type: type)
        TSCommentNetWorkManager.submitComment(for: commentType, content: commentContent, sourceId: sourceId, replyUserId: replyToUserId, contentType: contentType) { (comment, msg, status) in
            // 发送失败
    ...
    func sendMoment(commentContent: String, replyToUserId: Int?, feedId: Int, type: ReceiveInfoSourceType, complete: @escaping((_ message: String?, _ id: Int?, _ error: NSError?) -> Void)) {
        ...
        try! RequestNetworkData.share.textRequest(method: .post, path: path, parameter: parameters, complete: { (data: NetworkResponse?, status) in
            ...
            guard let commentDatas = data as? [String: Any], let comment = commentDatas["comment"] as? [String: Any] else {
                complete("error_decode_data_server_return".localized, nil, nil)
                return
            }
            // 3. 正常数据解析
    ...
    /// 获取评论数据
    func getPostCommetList(groupId: Int, postId: Int, after: Int?, complete: @escaping((_ isSuccess: Bool, _ commnetModels: [TSMomentCommentModel]?, _ error: String?) -> Void)) {
        // 配置路径
        ...
        try! RequestNetworkData.share.textRequest(method: .get, path: path, parameter: nil, complete: { (data: NetworkResponse?, status: Bool) in
            ...
            // 2. 服务器数据异常
            guard let datas = data as? [[String: Any]] else {
                complete(false, nil, "error_server_data".localized)
                return
            }
            // 3. 正常数据解析
            var commentObject: [TSMomentCommentModel] = []
            for comment in datas {
                let commentModel = TSMomentCommentModel(postComment: comment)
                var isHave = false
                let deleteCommentsTask = DatabaseManager().comment.getDeleteTask()
                if let deleteDatas = deleteCommentsTask {
                    for delete in deleteDatas {
...
extension TSCommentNetWorkManager {
    ...
    class func getMomentCommentList(type: TSCommentType, feedId: Int, afterId: Int?, limit: Int = TSAppConfig.share.localInfo.limit, complete: @escaping((_ commentList: [FeedCommentListCellModel]?, _ msg: String?, _ status: Bool) -> Void)) -> Void {

        var request = Request<FeedListCommentModel>(method: .get, path: "feeds/:feed/comments", replacers: [":feed"])
                ...
                case .success(let response):
                    ...
                    if var normalCommentList = Mapper<FeedListCommentModel>().mapArray(JSONObject: dataDic["comments"]) {
                        ...
                        if topComment.isEmpty {
                            ...
                            normalCommentList = normalCommentList.filter{ (comment) -> Bool in
                                return !topComment.contains(where: { $0.id == comment.id })
                            }
                            commentList += normalCommentList
                        }
                    }
                    ...
                    complete(returnComments, response.message, true)
            }
    ...
    class func getCommentList(type: TSCommentType, sourceId: Int, afterId: Int?, limit: Int, complete: @escaping((_ commentList: [TSCommentModel]?, _ msg: String?, _ status: Bool) -> Void)) -> Void {
        ...
        RequestNetworkData.share.text(request: request) { (result) in
            ...
            case .success(let response):
                ...
                // 资讯、动态、帖子的评论进行特殊处理
                if type == .momment || type == .post || type == .news {
    ...
    class func submitComment(for type: TSCommentType, content: String, sourceId: Int, replyUserId: Int?, contentType: CommentContentType, complete: @escaping ((_ comment: TSCommentModel?, _ msg: String?, _ status: Bool) -> Void)) -> Void {
        // 1. url
        var request: Request<TSCommentModel>
        switch type {
        case .momment:
    ...
    class func pinComment(for commentId: Int, sourceId: Int, complete: @escaping ((_ msg: String?, _ status: Bool) -> Void)) -> Void {
        
        var request: Request<Empty>
        request = CommentNetworkRequest.Moment.pinComment
        request.urlPath = request.fullPathWith(replacers: ["\(sourceId)", "\(commentId)"])
    ...
    class func unpinComment(for commentId: Int, sourceId: Int, complete: @escaping ((_ msg: String?, _ status: Bool) -> Void)) -> Void {
        
        var request: Request<Empty>
        request = CommentNetworkRequest.Moment.unpinComment
        request.urlPath = request.fullPathWith(replacers: ["\(sourceId)", "\(commentId)"])
