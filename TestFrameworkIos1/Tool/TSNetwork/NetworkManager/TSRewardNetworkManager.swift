...
class TSRewardNetworkManger {
    ...
    /// 打赏的网络请求
    class func reward(type: TSRewardType, sourceId: Any, rewardId: Int, complete: @escaping ((_ msg: String?, _ code: Int, _ status: Bool) -> Void)) -> Void {
        
        var parametars: [String: Any] = ["reward_id": rewardId]
        ...
        case .sticker:
            request = TSRewardNetworkRequest.Sticker.reward
        case .post:
            request = TSRewardNetworkRequest.Post.reward
        case .live(let messageId):
            request = TSRewardNetworkRequest.Live.reward
            parametars["msgID"] = messageId

            if let user = CurrentUserSessionInfo {
                parametars["request_id"] = user.requestKey
            }
        }
        request.urlPath = request.fullPathWith(replacers: ["\(sourceId)"])
        ...
        RequestNetworkData.share.text(request: request) { (result) in
            switch result {
            case .error(_):
                complete("network_problem".localized, -999, false)
            case .failure(let response):
                complete(response.message, response.statusCode, false)
            case .success(let response):
                complete(response.message, response.statusCode, true)
            }
        }
    }
}
