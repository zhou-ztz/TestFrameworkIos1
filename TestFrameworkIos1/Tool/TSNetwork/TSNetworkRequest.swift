...
enum VersionType: String {
    case normal = "api/v2/"
    case wallet = "wallet/api/"
    case referral = "api/member/"
}
...
struct Request<T: Mappable>: NetworkRequest {
    ...
    init(version: VersionType = .normal, method: HTTPMethod, path: String, replacers: [String]) {
...
struct TSNetworkRequest {
    ...
    let followersList = TSNetworkRequestMethod(method: .get, path: "users/{user}/new/followers", replace: "{user}")
    /// 用户关注列表
    /// 查询多个用户将 replace 替换为标识字符串
    let followingsList = TSNetworkRequestMethod(method: .get, path: "users/{user}/new/followings", replace: "{user}")
    /// 关注某用户
    ...
    let invitedFriends = TSNetworkRequestMethod(method: .get, path: "user/invited-users", replace: nil)
    //关注的商家用户
    let mechantFriends = TSNetworkRequestMethod(method: .get, path: "user/followings", replace: nil)
}
...
struct MiniProgram {
//    let authUser = Request<MiniProgramAccountModel>(method: .post, path: "oauth/client/{client_id}/token", replacers: ["{client_id}"])
//    let getUser = Request<UserInfoModel>(method: .get, path: "user/me", replacers: [])
}
